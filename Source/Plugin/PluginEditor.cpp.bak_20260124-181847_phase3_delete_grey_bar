...
void CompassMasteringLimiterAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    //// [CML:UI] GR module framing — recessed plate + well
    {
        ...
        constexpr float kOuterInsetPx   = 10.0f;
        constexpr float kCavityExpandPx = 18.0f;
        constexpr float kTabWidthFrac   = 0.18f;
        constexpr float kTabHeightPx    = 32.0f;
        constexpr float kTabRiseFrac    = 0.72f;

        constexpr float kOuterCornerPx  = 10.0f;
        constexpr float kCavityCornerPx = 9.0f;
        constexpr float kWellCornerPx   = 8.0f;

        constexpr float kPlateTopA      = 0.06f;
        constexpr float kPlateBotA      = 0.52f;
        constexpr float kPlateStrokeA   = 0.07f;

        constexpr float kCavityTopA     = 0.62f;
        constexpr float kCavityBotA     = 0.92f;
        constexpr float kCavityStrokeA  = 0.05f;
        constexpr float kBevelHiA       = 0.06f;
        constexpr float kBevelLoA       = 0.52f;
        ...
        constexpr float kGlassInsetPx   = 2.0f;
        constexpr float kGlassHeightFrac= 0.45f;
        constexpr float kGlassTopA      = 0.06f;
        ...
        {
            juce::ColourGradient plate (juce::Colours::white.withAlpha (kPlateTopA),
                                        outer.getCentreX(), outer.getY(),
                                        juce::Colours::black.withAlpha (kPlateBotA),
                                        outer.getCentreX(), outer.getBottom(),
                                        false);
            g.setGradientFill (plate);
            g.fillRoundedRectangle (outer, kOuterCornerPx);

            g.setColour (juce::Colours::white.withAlpha (kPlateStrokeA));
            g.drawRoundedRectangle (outer, kOuterCornerPx, 1.0f);
        }

        // 2) Inner cavity bezel (darker gradient + bevel strokes)
        {
            juce::ColourGradient cav (juce::Colours::black.withAlpha (kCavityTopA),
                                      cavity.getCentreX(), cavity.getY(),
                                      juce::Colours::black.withAlpha (kCavityBotA),
                                      cavity.getCentreX(), cavity.getBottom(),
                                      false);
            g.setGradientFill (cav);
            g.fillRoundedRectangle (cavity, kCavityCornerPx);

            g.setColour (juce::Colours::white.withAlpha (kCavityStrokeA));
            g.drawRoundedRectangle (cavity, kCavityCornerPx, 1.0f);
            ...
            g.setColour (juce::Colours::white.withAlpha (kBevelHiA));
            g.drawLine (x1, y1, x2, y1, 1.0f);
            g.drawLine (x1, y1, x1, y2, 1.0f);

            g.setColour (juce::Colours::black.withAlpha (kBevelLoA));
        ...
        // 4) Glass overlay (clip to well, top highlight) — reduced to avoid “widget sheen”
        {
            juce::Graphics::ScopedSaveState ss (g);
            g.reduceClipRegion (well.toNearestInt());

            auto glass = well.reduced (kGlassInsetPx, kGlassInsetPx);
            glass.setHeight (glass.getHeight() * kGlassHeightFrac);

            juce::ColourGradient grad (juce::Colours::white.withAlpha (kGlassTopA),
                                       glass.getCentreX(), glass.getY(),
                                       juce::Colours::white.withAlpha (0.0f),
                                       glass.getCentreX(), glass.getBottom(),
                                       false);
            g.setGradientFill (grad);
            g.fillRoundedRectangle (glass, (kWellCornerPx - 1.0f));
        }
