            ...
            for (int iN = 0; iN < n; ++iN)
            {
                ...
                for (int k = 0; k < osFactor; ++k)
                {
                    ...
                    for (int c = 0; c < chProc; ++c)
                    {
                        ...
                        // Adaptive Release inputs (continuous, bounded, deterministic)
                        // Sustained energy: macro01 (already bounded)
                        const double sustained01 = juce::jlimit (0.0, 1.0, macro01);

                        // Crest factor proxy: short RMS accumulator of |s| (bounded mapping to [0,1])
                        constexpr double kCrestRmsSecBase = 0.050; // 50 ms nominal
                        const double crestRmsSec = kCrestRmsSecBase * (1.15 - 0.30 * bias01); // continuous responsiveness
                        const double aCR = std::exp (-(dtOS) / juce::jmax (1.0e-6, crestRmsSec));
                        ...
                        const double crest01 = juce::jlimit (0.0, 1.0, (crest - 1.0) / (crest + 1.0)); // 0..1

                        // Event density: continuous event presence (no switches), derived from attenuation demand
                        constexpr double kDensitySecBase = 0.090; // 90 ms nominal
                        const double densitySec = kDensitySecBase * (1.20 - 0.40 * bias01); // bias affects responsiveness
                        const double aD = std::exp (-(dtOS) / juce::jmax (1.0e-6, densitySec));
                        const double densityInput = 1.0 - std::exp (-0.18 * attnTargetDb); // 0..~1 continuous
                        ...
                        // Adaptive release curve (continuous, bounded, deterministic)
                        // - Only depends on: crest01, density01, sustained01 (plus bias scalar for weighting)
                        constexpr double kMicroSecMin = 0.0030; // 3 ms
                        constexpr double kMicroSecMax = 0.0600; // 60 ms
        ...
        else
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                for (int c = 0; c < chProc; ++c)
                {
                    ...
                    constexpr double kCrestRmsSecBase = 0.050; // 50 ms nominal
                    const double crestRmsSec = kCrestRmsSecBase * (1.15 - 0.30 * bias01); // continuous responsiveness
                    const double aCR = std::exp (-(lastInvSampleRate) / juce::jmax (1.0e-6, crestRmsSec));
                    const double absS = std::abs ((double) s);
                    crestRmsSqState[(size_t) c] = aCR * crestRmsSqState[(size_t) c] + (1.0 - aCR) * (absS * absS);
                    const double rmsLin = std::sqrt (juce::jmax (0.0, crestRmsSqState[(size_t) c]));
                    const double crest = absS / (rmsLin + 1.0e-12);
                    const double crest01 = juce::jlimit (0.0, 1.0, (crest - 1.0) / (crest + 1.0)); // 0..1
                    ...
                    constexpr double kDensitySecBase = 0.090; // 90 ms nominal
                    const double densitySec = kDensitySecBase * (1.20 - 0.40 * bias01); // bias affects responsiveness
                    const double aD = std::exp (-(lastInvSampleRate) / juce::jmax (1.0e-6, densitySec));
                    const double densityInput = 1.0 - std::exp (-0.18 * attnTargetDb); // 0..~1 continuous
                    eventDensityState[(size_t) c] = aD * eventDensityState[(size_t) c] + (1.0 - aD) * densityInput;
                    const double density01 = juce::jlimit (0.0, 1.0, eventDensityState[(size_t) c]);
