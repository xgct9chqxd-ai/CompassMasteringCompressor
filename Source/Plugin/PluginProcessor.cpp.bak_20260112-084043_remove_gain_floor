...
void CompassMasteringLimiterAudioProcessor::reset (double sampleRate, int maxBlock, int channels) noexcept
{
    ...
    // Gate-5: reset GR gain smoothing state.
    grGainState = 1.0f;
}
...
void CompassMasteringLimiterAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    // Control-domain true peak measurement (oversampled reconstruction, per channel).
    measureTruePeak (buffer);

    // Gate-5: Core GR Law in dB (continuous + differentiable).
    // All GR computation stays in dB domain; mapping is smooth and monotonic with Drive.
    // Hardening: stable, branch-free softplus approximation of max(0, x) in dB (C-infinity; no steps).
    // Robustness: clamp extreme attenuation + floor the linear gain to avoid ultra-tiny multiplies.
    {
        const int numCh = juce::jmin (getTotalNumInputChannels(), buffer.getNumChannels());
        const int n     = buffer.getNumSamples();

        // Convert block true-peak (linear) to dB once per block.
        constexpr double kEpsLin = 1.0e-12; // avoids log(0)
        const double tpDb = 20.0 * std::log10 (truePeakLin + kEpsLin);

        // Controls transition sharpness (higher = closer to hard max while remaining smooth).
        constexpr double kSoftK = 32.0;

        // Practical bounds for extreme cases (does not affect normal operation).
        constexpr double kMaxAttnDb = 120.0;   // cap attenuation to avoid pathological tiny gains
        constexpr float  kMinGain   = 1.0e-6f; // -120 dB floor

        // Cache write pointers to avoid per-sample getSample/setSample overhead (hot path).
        // No allocations; falls back safely if channel count exceeds cache.
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            const double x = (tpDb + driveDb) - ceilingDb; // dB "over ceiling" after Drive

            // Stable, branch-free softplus (numerically robust for extreme z).
            const double z = kSoftK * x;
            const double softplus = z + std::log1p (std::exp (-std::abs (z)));

            double attnDb = softplus / kSoftK;

            // Clamp to practical range to prevent pathological tiny gains in extreme attenuation.
            attnDb = juce::jlimit (0.0, kMaxAttnDb, attnDb);

            const float gTarget = juce::jmax (kMinGain, (float) std::pow (10.0, -attnDb / 20.0));

            // Gate-5 hardening: smooth GR gain to prevent block-boundary discontinuities/clicks.
            // Attack: fast when reducing gain (more attenuation). Release: slower when restoring gain.
            constexpr float kAttackSec  = 0.001f; // 1 ms
            constexpr float kReleaseSec = 0.050f; // 50 ms

            const float sr = (float) juce::jmax (1.0, lastSampleRate);
            const float aA = (float) std::exp (-1.0f / (sr * kAttackSec));
            const float aR = (float) std::exp (-1.0f / (sr * kReleaseSec));
            const float a  = (gTarget < grGainState) ? aA : aR;

            grGainState = gTarget + a * (grGainState - gTarget);

            const float g = juce::jmax (kMinGain, grGainState);

            for (int c = 0; c < chCached; ++c)
                chPtr[(size_t) c][i] *= g;

            for (int c = chCached; c < numCh; ++c)
